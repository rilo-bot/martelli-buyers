...
documentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!to) {
      res.status(400).json({ error: 'No client email on the linked campaign.' });
      return;
    }
    const buf = await buildInvoicePdf(invoice.toJSON() as never, deal!.toJSON() as never);
    await sendMail({
      to,
      subject: `Invoice ${invoice.invoiceNumber} from Martelli Buyers Agents`,
      text: `Hi ${deal!.clientName || 'there'},\n\nPlease find attached invoice ${invoice.invoiceNumber} for ${invoice.description || 'buyer agency services'}.\n\nKind regards,\nMartelli Buyers Agents`,
      attachments: [{ filename: `${invoice.invoiceNumber || 'invoice'}.pdf`, content: buf, type: 'application/pdf' }],
    });
...
documentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!deal.clientEmail) {
      res.status(400).json({ error: 'No client email on this campaign.' });
      return;
    }
