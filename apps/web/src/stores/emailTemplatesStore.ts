...
interface EmailTemplatesState {
  ...
  /**
   * Server seeds nothing for templates — bulk-create the defaults. Seeding is
   * tombstoned: the server records every default name ever seeded, so a default
   * the user later deletes is NOT re-created, while genuinely new defaults added
   * to the codebase still seed once into existing workspaces.
   */
...
export const useEmailTemplatesStore = create<EmailTemplatesState>()((set, get) => ({
  ...
  seedDefaultTemplates: async () => {
    // Names ever seeded into this workspace (the tombstone set). If the endpoint
    // is unavailable, fall back to an empty set — the existing-name guard below
    // still prevents duplicates, matching the previous backfill behaviour.
    let seededNames: string[] = [];
    try {
      const state = await request<{ seededNames: string[] }>('GET', '/api/email/seed-state');
      seededNames = state.seededNames ?? [];
    } catch {
      seededNames = [];
    }
    ...
    // Defaults never recorded as seeded. We create only those not already
    // present, but record ALL untracked names — so on a workspace seeded before
    // tombstoning, the existing defaults get tracked and any later deletion of
    // them now sticks.
    const untracked = DEFAULT_TEMPLATES.filter((t) => !seeded.has(t.name));
    if (untracked.length === 0) return;

    const toCreate = untracked.filter((t) => !existingNames.has(t.name));
    if (toCreate.length > 0) {
      const created = await Promise.all(toCreate.map((t) => templatesApi.create(t)));
      set((s) => ({ templates: [...s.templates, ...created] }));
    }

    // Record the tombstone (server unions with what it already has). Non-fatal:
    // a failure just means a future load retries the same idempotent work.
    try {
      await request('PATCH', '/api/email/seed-state', { names: untracked.map((t) => t.name) });
    } catch {
      /* best-effort */
    }
  },
